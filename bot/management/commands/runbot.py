...
class Command(BaseCommand):
    """
    Создание команды для manage.py
    Наследуется от класса BaseCommand

    handle_user_without_verification: принимает в себя данные о неверифициированном пользователе (его id, имя пользователя и id чата) и отдает
    в сообщении код для верификации на сайте

    fetch_tasks: принимает в себя данные о пользователе и отдает список задач, которые он создал на сайте

    handle_verified_user: принимает в себя данные о верифицированном пользователе и о сообщении которое он отправил, если пользователь ввел команду
    из доступных, то бот ответит ожидаеммыми данными, иначе ответит сообщением 'unknown command'
